...
//	Start:
	system("cls");
	printRusComment("Путь, если необходимо, нужно вводить с ковычками. Можно \"перетащить\" файл в консоль для получения пути\n");
	Opening:;
	sprintf(tmp, "Введите путь до файла (стереокартинки/видео для разделения на два изображения/видео): ");
	printRusComment(tmp);
	read(file1, stdin);
	if (!Analize(file1)) {
...
//	Work:;
	...
	printf("%s\n", str);
	if (system(str)) {
		quit("Ошибка при работе ffmpeg'а - создание второго файла");
	}
	if (u)
		goto Opening;
